...
/// 进度条悬浮缩略图预览。
///
/// 输入一个播放位置 [scrubPosition]，从 [thumbnails] 上调
/// [ThumbnailController.thumbnailFor] 取出 frame，用 [NiumaThumbnailView]
/// 渲染成 [size] 所声明的目标尺寸；可选的时间标签 [showTime]（默认 `true`）
/// 在缩略图下方画一行 `mm:ss` 文字。
///
/// 当 [ThumbnailController.thumbnailFor] 返回 `null`（VTT 未加载 / 不命中）时，
/// 本 widget 直接返回 [SizedBox.shrink]，不占布局空间——ScrubBar 上层根据
/// `source.thumbnailVtt` 是否声明再决定是否挂载本组件。
///
...
class NiumaScrubPreview extends StatelessWidget {
  ...
  /// 提供 [ThumbnailController.thumbnailFor] 的缩略图取帧协调器。
  ...
  Widget build(BuildContext context) {
    final frame = thumbnails.thumbnailFor(scrubPosition);
