...
class _M12PipDemoPageState extends State<M12PipDemoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    //
    // 双信号判定：isInPictureInPicture（Dart 侧 PipModeChanged 事件 +
    // 乐观更新）OR LayoutBuilder 检测到容器宽 <400dp（=PiP 窗或极小内嵌）。
    // 任一为真就走 PiP 布局——避开 Android "Activity 先 resize 后回事件"
    // 的时序竞争。
    return LayoutBuilder(
      builder: (lbCtx, constraints) {
        final tooNarrow = constraints.maxWidth < 400;
        return AnimatedBuilder(
          animation: _video,
          builder: (ctx, _) {
            final inPip = _video.value.isInPictureInPicture || tooNarrow;
            return Scaffold(
