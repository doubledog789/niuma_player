...
| 日期 | 2026-04-27 |
| 前置 | M3.x 内核完成（kernel 已就绪：state machine + Try-Fail-Remember + 三端路由） |
...
新增 4 个 milestone（M3 之后 / M4-M5 仍待定）：
...
总计串行约 15d；M6 并行后 wall-clock 约 15–17d。

每个 milestone 内部包含**自身的测试**（kernel 之外的所有新代码都自带单测）。测试不单独立项。
